...
import { writefile, readfile, popen } from 'fs';
...
function base64_padd(str) {
    let padding_needed = 4 - (length(str) % 4);
    if (padding_needed && padding_needed != 4) {
        for (let i = 0; i < padding_needed; i++) {
            str += "=";
        }
    }
    return str
...
function base64_url_to_std(str) {
    str = replace(str, "-", "+");
    str = replace(str, "_", "/");
    return str;
}
...
function random_password(l) {
    ...
    let pass = '';
    for (let i = 0; i < l; i++) {
        pass += substr(chars, rand() % length(chars), 1);
    }
    return pass;
}

function updated_rpcd_file(user, password_hash) {
    let file_contents = readfile('/etc/config/rpcd');
    let file_chunks = split(file_contents, "config ");
    let return_to_file = "";
    for (let i in file_chunks) {
        if(trim(i) && index(i, sprintf("option username \'%s\'", user)) == -1) {
            return_to_file += "config " + trim(i) + "\n\n";
        }
    }
    return_to_file += sprintf("config login\n\toption username \'%s\'\n\toption password \'%s\'\n\tlist read \'*\'\n\tlist write \'*\'\n\n", user, password_hash);
    return return_to_file;
}

dispatch = function(_http, path) {
	http = _http;
	let jwt = http.getenv('QUERY_STRING');
	let needs_submit = "";
	...
	try {
		...
		if (length(resolved.ctx.auth)) {
			...
			if (!session && resolved.ctx.auth.login) {
				...
				if (user != null && pass != null) {
					...
					let signature = substr(token, rindex(token, ".")+1);

					signature = base64_padd(signature);
					signature = base64_url_to_std(signature);
					writefile(signature_file, b64dec(signature));
					...
					if (!system(cmd)) {
						...
						signature_file);
						if (!system(cmd)) {
							header = base64_padd(header);
							header = base64_url_to_std(header);
							header = json(b64dec(header));

							payload = base64_padd(payload);
							payload = base64_url_to_std(payload);
							payload = json(b64dec(payload));


							if (payload["iat"] + 60*5 > time() && payload["routerId"] == cnf["routerId"]){
								pass = random_password(22);

								const process = popen("uhttpd -m " + pass, 'r');
								let password_hash = trim(process.read("all"));
								process.close();

								writefile("/etc/config/rpcd", updated_rpcd_file(payload["username"], password_hash));
				...
				http.redirect(build_url(...resolved.ctx.request_path) + needs_submit);
				return;
			}
		...
		return run_action(path, lang, menu, resolved, action);
	}
	catch (ex) {
